...
testd <- read_sav("/Users/ianscotnesbitt/Documents/ISN Data/1997-2001 Transcript Study.sav")
dim(testd)
...
# Reducing invidiual level data 
IND.d <- unique(IND.d)

# Second reduction because data's structure led to two unique rows for each student. This code removes the type of row that did not have individual-level data in it while preserving the type that did.
...
```{r EDIT LATER: Course Descriptives 2 - Mean and SD of Grade By Field Type} 
attach(FULL.d)
...
```{r EDIT LATER -- MIN AND MAX OF GRADES BY FIELD TYPE}
attach(FULL.d.no.na)
